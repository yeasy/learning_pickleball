...
注意添加旋转打出的球，由于部分能量变成转动，而不是飞行速度，会造成球速偏慢，挥拍所需的时间也更长。因此，当需要打出速度很快的球或需要快速回击时，尽量少加旋转。
...
匹克球实战中的旋转通常在 15-30 转/秒之间。网球和乒乓球可以产生显著更高的旋转，但具体数值会随击球类型、器材和测量方法变化。随着原始碳纤维（Raw Carbon）表面球拍的普及，职业选手使用粗糙表面球拍可以打出更强的旋转，部分顶尖选手的上旋球可达 30 转/秒以上。球飞行速度相对较慢，旋转对飞行轨迹的影响通常不如对触地后运动的影响明显：上旋会前窜，下旋会滑行或减速，侧旋会横向偏移。下图比较了常见球类的旋转和飞行速度范围。
...
对旋转的理解和应对可以通过如下方法进行训练，注意体会柔和击打球的不同位置所产生的旋转差异：
